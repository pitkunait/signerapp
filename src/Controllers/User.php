...
class User{
    ...
    public function delete() {
        ...
        //check for documents & folders (temporary solution)
        $filesuploaded = Database::table("files")->where("uploaded_by" , $account->id)->count("id", "files")[0]->files;
        $folderscreated = Database::table("folders")->where("created_by" , $account->id)->count("id", "folders")[0]->folders;
        if ($filesuploaded > 0 || $folderscreated > 0) {
            header('Content-type: application/json');
            exit(json_encode(responder("error", "Error!", "Unable to delete record, the user has documents/folders in the system.")));
        }
