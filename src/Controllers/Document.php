...
class Document{
    ...
    public function open($document_key) {
        ...
        return view('sign', compact("user", "document", "history", "customers", "team", "chats", "company","fields","inputfields","templateFields","savedWidth","lauchLabel","request","requestWidth","requestPositions","stamps"));
    }
    ...
    public function fetch() {
        ...
        $folders = $documents = $subfolders = array();
        ...
        if (empty($_POST['type']) || $_POST['type'] == "folders") {
            ...
            if(input("folder") != 1){
                $subfolders = Database::table("folders")->where("company", $user->company)
                    ->where("id", input("folder"))->groupBy("folder")->orderBy("id", false)->get();
            }
            ...
            foreach ($subfolders as $key => $subfolder) {
                if ($user->role == "user" && $subfolder->created_by != $user->id) {
                    unset($folders1[$key]);
                }
                if ($user->id == $subfolder->created_by || $subfolder->accessibility == "Everyone") {
                    continue;
                }
                if ($subfolder->accessibility == "Only Me" && $user->id != $subfolder->created_by) {
                    unset($folders1[$key]);
                }
                $giveAccess = false;
                if ($subfolder->accessibility == "Departments") {
                    $allowedDepartments = json_decode($subfolder->departments);
                    foreach ($allowedDepartments as $department) {
                        $userDepartments = Database::table("departmentmembers")->where("department", $department)->where("member", $user->id)->get("department");
                        if (count($userDepartments) > 0) {
                            $giveAccess = true;
        ...
        if (empty($_POST['type']) || $_POST['type'] == "files") {
            ...
            if (input("folder") == "1") {
                ...
                if(!empty($requestsData)){
                    foreach($requestsData as $request){
                        $sharedFileKeys[] = $request->document;
                    }
                }
            }
            if (!empty(input("search"))) {
                ...
                $documents = Database::table("files")->where("company", $user->company)
                                                  ->where("folder", input("folder"))->where("is_template", "No")->where("id", "NOT IN", "('".implode("', '", $hiddenFiles)."')")->orWhere("document_key", "IN", "('".implode("', '", $sharedFileKeys)."')")->where("id", "NOT IN", "('".implode("', '", $hiddenFiles)."')")->orderBy("id", false)->get();
            }
            foreach ($documents as $key => $document) {
                ...
                if(!in_array($document->document_key, $sharedFileKeys)){
        ...
        return view('extras/documents', compact("user", "folders", "documents","subfolders"));
    }
    ...
    public function dropboximport() {
        ...
        $data = array(
        				...
        				"source" => "url",
        				"document_key" => Str::random(32),
        				"activity" => 'File Imported from Dropbox by <span class="text-primary">'.$actionTakenBy.'</span>.'
        			);
        $upload = Signer::upload($data);
        if ($upload['status'] == "success") {
	        exit(json_encode(responder("success", "", "","documentsCallback()", false)));
    ...
    public function googledriveimport() {
        ...
        $data = array(
        				...
        				"activity" => 'File Imported from Google Drive by <span class="text-primary">'.$actionTakenBy.'</span>.'
        			);
        $upload = Signer::upload($data);
        header('Content-type: application/json');
        if ($upload['status'] == "success") {
    ...
    public function restore() {
        ...
        $activity = 'File restored to orginal version by <span class="text-primary">'.$actionTakenBy.'</span>';
    ...
    public function replace() {
        ...
        if ($upload['status'] == "success") {
            ...
            $activity = 'File replaced with a new one by <span class="text-primary">'.$actionTakenBy.'</span>.';
