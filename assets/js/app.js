...
$(document).ready(function() {
  ...
  // sidebar - scroll container
	...
	/*
	 * guest signing switch
	 */
    $("input[name=GUEST_SIGNING]").change(function() {
        if ($(this).prop("checked")) {
            $(".force-guest-sign").show();
        } else {
            $(".force-guest-sign").hide();
        }
...
// Switch login cards
$(".login-card a").not('.singing_as_guest').click(function(event) {
    event.preventDefault();
...
$(".add-reminder").click(function(){
    ...
    $(".reminders-holder").append('<div class="panel panel-default"><div class="panel-heading"><span class="delete-reminder" data-toggle="tooltip" title="Remove reminder"><i class="ion-ios-trash"></i></span><h4 class="panel-title"><a data-parent="#accordion" data-toggle="collapse" href="#collapse-'+ reminderKey +'">Reminder #<span class="count">' + reminderNumber +'</span></a></h4></div><div class="panel-collapse collapse in" id="collapse-'+ reminderKey +'"><div class="panel-body"><div class="remider-item"><div class="form-group"><div class="col-md-12 p-lr-o"><input type="hidden" name="count[]" value="1"><label>Email subject</label> <input class="form-control" name="subject[]" placeholder="Email subject" required="" type="text" value="Signing invitation reminder"></div></div><div class="form-group"><div class="col-md-12 p-lr-o"><label>Days after request is sent</label> <input class="form-control" name="days[]" min="1" placeholder="Days after request is sent" required="" type="number" value="7"></div></div><div class="form-group"><div class="col-md-12 p-lr-o"><label>Message</label><textarea class="form-control" name="message[]" required="" rows="9">Hello there, I hope you are doing well. I am writing to remind you about the signing request I had sent earlier.Cheers!'+fullName+'</textarea></div></div></div></div></div></div>');
 ...
 */
 function saveTextSignature(){
    domtoimage.toBlob(document.getElementById('text-signature')).then(function(blob) {
...
/*
* Hide "Edit Signature" modal on button click
* */
$("body").on("click", ".edit-signature", function () {
    $("#editDrawModal").modal('hide');
});

/*
* When edit signature modal hide everytime call this
* */
$('#editDrawModal').on('hidden.bs.modal', function (e) {
    // do something here
    //signaturePadObj.penColor = "#000000";
