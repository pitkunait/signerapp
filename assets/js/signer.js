...
function isEmail(email) {
 ...
 /*
  * Before an email is added to request form
  */
 $('input[name=recipients]').on('beforeItemAdd', function(event) {
  if (!isEmail(event.item)) {
    event.cancel = true;
    toastr.error("Enter a valid email address.","Oops!");
  }else{
    $.ajax({
      url: '/users/checkemail/',   // sending ajax request to this url
      type: 'post',
      data: {
        'email': event.item,
      ...
      success: function(response) {
        ...
        {
          event.cancel = true;
          $('input[name=recipients]').tagsinput('remove', event.item);
          toastr.error("Email address not found.","Oops!");
        }
      }
    });
    }
...
/*
* Before an email is added to send form
  */
 $('input[name=receivers]').on('beforeItemAdd', function(event) {
  if (!isEmail(event.item)) {
      event.cancel = true;
      toastr.error("Enter a valid email address.","Oops!");
    }
...
$('input[name=recipients]').on('itemRemoved', function(event) {
  ...
  */
  function requestOptions(){
    ...
    if (recipients.length > 1) {
        $(".duplicate-request, .chain-request").show();
      
        if ($("input[name=duplicate]").prop("checked")) {
            $(".chain-request").hide();
            if ($("input[name=chain]").prop("checked")) {
                $("input[name=chain]").click();
            }
        }else{
            $(".chain-request").show();
        }
    }else{
      $(".duplicate-request, .chain-request").hide();
    }
  }
  ...
  */
  function validateRequest(){
    if ($("input[name=restricted]").prop("checked")) {
      ...
      enableTools("request");
      $(".request-helper").show();
      $(".request-helper strong").text(recipients[0]);
  ...
  */
  function nextRecipient(){
      var recipientIndex = parseInt($(".signer-next").attr("index"));
      if ((recipientIndex + 1) >= recipients.length) {
          $(".signer-next").hide();
          $(".signer-save").show();
      }
      $(".signer-next").attr("index", parseInt($(".signer-next").attr("index")) + 1);
      orgnizeData(false);
      modules.positions.push(prepareData(false));
      emptyBuilder()
      emptyAssembler()
      $(".right-bar.input-fields-list").toggleClass("open");
      $(".request-helper strong").text(recipients[recipientIndex]);
  }
  ...
  */
  function sendRequest(){
    var emails = JSON.stringify($("input[name=recipients]").tagsinput('items')),
          ...
          duplicate = chain = "No", positions = docWidth = '';
    if ($("input[name=duplicate]").prop("checked") && emails.length > 1) { duplicate = "Yes"; }
    if ($("input[name=chain]").prop("checked") && emails.length > 1) { chain = "Yes"; }
    if (isTemplate === "Yes" && templateFields !== null) {
      positions = JSON.stringify(templateFields); 
      docWidth = "set";
    }else if(isTemplate === "Yes" && templateFields == null){
        orgnizeData(false);
        modules.positions.push(prepareData(false));
        positions = JSON.stringify(modules.positions);
        docWidth = $("#document-viewer").width();
    }else if($("input[name=restricted]").prop("checked") && emails.length == 1){
      orgnizeData(false);
...
$('#send-team-action-send .col-md-12, #send-customers-action-send .col-md-12').slimscroll({
    height: '200px',
    width: '100%',
    size: "3px",
    color: 'rgba(0, 0, 0, 0.8)',
...
function getChats() {
    ...
    posting.done(function(data) {
      if (data != 'empty') {
        ...
        if(!$(".right-bar.chat-holder").hasClass("open")){
          $(".right-bar.chat-holder").addClass("open");
        }
      }
    });
}
...
$(".signer-tool").click(function(event){
  ...
  if (tool === "rotate") {
    ...
    $(".right-bar.stamp-list").toggleClass("open");
  }else if(tool === "fields"){
    if (auth) {
  ...
  if (prepare === undefined) { prepare = true; }
    ...
    $('.signer-builder .signer-element').each(function(index, value) {
      ...
      if (actionType === "image" || actionType === "signature" || actionType === "symbol" || actionType === "shape" || actionType === "stamp") {
        ...
        if (actionType === "symbol" || actionType === "shape" || actionType === "stamp") {
...
function saveChanges(actions){
  server({
      url: signDocumentUrl,
      data: {
          "actions": btoa(unescape(encodeURIComponent(actions))),
          "docWidth": $("#document-viewer").width(),
          "document_key": document_key,
...
function selectDocImage(){
  ...
  reader.addEventListener("load", function () {
    ...
    $( document ).mousemove(function( event ) {
      $(".signer-element[status=drop]").css({ left:  event.pageX + 3, top:   event.pageY + 3  });
    });
    disableTools();
    highlightCanvas();
  }, false);
...
/*
 *  When stamp is selected
 */
$(".stamp-list").on("click", ".stamp-item",function(){
  deselectElements();
  if ($(this).hasClass("saved")) {
    $(".delete-stamp").show();
    $(".delete-stamp").attr("data-id", $(this).attr("data-id"));
    $(".save-stamp").hide();
  }else{
    $(".delete-stamp").hide();
    $(".save-stamp").show();
  ...
  $(".right-bar.stamp-list").toggleClass("open");
  $(".stamp-preview").html($(this).html());
  $("#editStamp").modal({show: true, backdrop: 'static', keyboard: false});
});

/*
 *  When stamp document
 */
$(".stamp-document").click(function(){
  deselectElements();
  $("#editStamp").modal("hide");
  $(".right-bar.stamp-list").toggleClass("open");
  document.getElementById('stamp-color').jscolor.fromString("000000");
  var stamp = $(".stamp-preview").html();
  $('<div class="signer-element selected-element" status="drop" resizeable="true" type="stamp" page="'+pageNum+'" style="width:150px;">'+stamp+'</div>').appendTo(".signer-builder");
  $( document ).mousemove(function( event ) {
    $(".signer-element[status=drop]").css({ left:  event.pageX + 3, top:   event.pageY + 3  });
  });
  if ($("input[name=savestamp]").prop("checked")) {
    stampId = random();
    $(".stamp-list.grid").prepend('<div class="stamp-item stamp-no-'+stampId+'">'+stamp+'</div>');
    $("input[name=savestamp]").click();
      server({
        url: saveFieldsUrl,
        data: {
            "fieldId": stampId,
...
/*
 *  When stamp is deleted
 */
$(".delete-stamp").click(function(event){
  event.preventDefault();
  stampId = $(this).attr("data-id");
  $("#editStamp").modal("hide");
  $(".stamp-"+stampId).remove();
  deleteField(stampId);
  toastr.success("Stamp deleted.", "Deleted!");
});
 ...
 */
 function updateStampColor(color){
  $(".stamp-preview").find("[signer-fill=true]").attr("fill", "#"+color);
  $(".stamp-preview").find("[signer-stroke=true]").attr("stroke", "#"+color);
 }


/*
 *  Adjusting stamp rotation
 */
 $(".rotate-stamp").change(function(){
  rotateStamp($(this).val());
 });

/*
 *  Adjusting stamp rotation
 */
 $(".rotate-stamp").keypress(function(){
  rotateStamp($(this).val());
 });
 ...
 */
 function rotateStamp(degrees){
  $(".stamp-preview svg").css("transform", "rotate("+degrees+"deg)");
  $(".stamp-preview svg").attr("rotate", degrees);
 }
 ...
 *  Stamp response
 */
 function stampResponse(stampKey, stampId){
    $('.stamp-list').find(".stamp-no-"+stampKey).attr("data-id", stampId);
    $('.stamp-list').find(".stamp-no-"+stampKey).addClass("saved stamp-"+stampId);
 }
...
function acceptRequest(){
    if ($(".signer-builder").is(':empty')) {
      $(".requested-fields").empty();
      if ($("body").hasClass("accept") && requestPositions.length) {
        ...
        $.each( requestPositions, function( i, field ) {
          ...
          if (field.type == "image") {
            ...
            elementId = field.text.replace(/[^a-z0-9\s]/gi, '').replace(/[_\s]/g, '-');
            if($.inArray( field.text , inputsTypes ) < 0){
              textInputs.push({ label: field.text, element: elementId });
            //   return;
            };
            inputsTypes.push(field.text);
            if (field.align !== '') { field.align = ' align="'+field.align+'"'; }
            if (field.strikethrough !== '') { field.strikethrough = ' strikethrough="true"'; }
            if (field.underline !== '') { field.underline = ' underline="true"'; }
            if (field.bold !== '') { field.bold = ' bold="true"'; }
